...
# --- NEW: A more professional, styled PDF class ---
class PDF(FPDF):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.WIDTH = 297  # A4 Landscape width
        self.HEIGHT = 210 # A4 Landscape height
        # --- FIX: Renamed variables to avoid conflict with FPDF internals ---
        self.theme_primary_color = (74, 144, 226)
        self.theme_text_color = (220, 220, 220)
        self.theme_bg_color = (30, 30, 30)
        self.theme_subtle_text_color = (180, 180, 180)

    def header(self):
        self.set_fill_color(*self.theme_primary_color)
        self.rect(0, 0, self.WIDTH, 12, 'F')
        self.set_font('Arial', 'B', 12)
        self.set_text_color(255, 255, 255)
        self.cell(0, 10, 'SOC Alert Investigation Report', 0, 1, 'C')
        self.ln(5)
    ...
    def footer(self):
        ...
        self.cell(0, 10, f"Report Generated: {datetime.now(timezone.utc).astimezone(pytz.timezone('Asia/Kolkata')).strftime('%Y-%m-%d %H:%M:%S IST')}", 0, 0, 'R')


    def add_page(self, orientation=''):
        super().add_page(orientation)
        self.set_fill_color(*self.theme_bg_color)
        self.rect(0, 0, self.WIDTH, self.HEIGHT, 'F')
        self.set_text_color(*self.theme_text_color)
    ...
    def key_value_pair(self, key, value):
        ...
        self.set_text_color(*self.theme_subtle_text_color)
...
@st.cache_data
def generate_investigation_pdf(_inv):
    """Generates a professional, landscape PDF report from the investigation dictionary."""
    pdf = PDF(orientation='L')
    pdf.add_page()
    
    alert = _inv.get('alert_row', {})
    
    # --- SIEM Summary Panel in a "Card" ---
    pdf.chapter_title(f"Summary for User: {_inv['user']} on {_inv['day']}")
    pdf.key_value_pair("Alert Hybrid Score", f"{alert.get('hybrid_score', 0):.4f}")
    rules = _inv.get('triggered_rules', [])
    pdf.key_value_pair("Triggered Rules", f"{', '.join(rules) if rules else 'None'}")
    
    ldap_df = _inv.get('ldap')
    if ldap_df is not None and not ldap_df.empty:
        ldap_info = ldap_df.iloc[0]
        pdf.key_value_pair("Employee Name", f"{ldap_info.get('employee_name', 'N/A')}")
        pdf.key_value_pair("Role", f"{ldap_info.get('role', 'N/A')}")
    
    psych_df = _inv.get('psychometric')
    if psych_df is not None and not psych_df.empty:
        psych_info = psych_df.iloc[0]
        pdf.key_value_pair("Psychometric Profile", f"O:{psych_info.get('O','-')} C:{psych_info.get('C','-')} E:{psych_info.get('E','-')} A:{psych_info.get('A','-')} N:{psych_info.get('N','-')}")
    pdf.ln(5)

    # --- AI Summary ---
    pdf.chapter_title("AI Generated Summary")
    pdf.chapter_body(_inv.get("summary", "No summary available."))
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if shap_fig:
            pdf.chapter_title("Model Explanation (SHAP)")
            shap_path = os.path.join(temp_dir, "shap.png")
            
            # --- FIX: Generate a wider, higher-resolution image for landscape layout ---
            shap_fig.write_image(shap_path, width=1000, height=400, scale=2)
            
            # --- FIX: Use the full page width (minus margins) when placing the image ---
            pdf.image(shap_path, w=pdf.WIDTH - 20)
            pdf.ln(5)

        # Event Sequence Chart
        events_fig = create_event_plotly_graph(_inv['events'])
        if events_fig:
            pdf.chapter_title("Event Sequence")
            events_path = os.path.join(temp_dir, "events.png")
            events_fig.write_image(events_path, width=1200, height=450, scale=2)
            pdf.image(events_path, w=pdf.WIDTH - 20)
            pdf.ln(5)
...
def show_investigation_ui(inv):
    ...
    pdf_bytes = generate_investigation_pdf(inv)
    
    st.download_button(
        label="📄 Download as PDF Report",
        data=pdf_bytes,
        file_name=f"investigation_{inv['user']}_{inv['day']}.pdf",
...
if st.session_state.run_complete:
    ...
    with tab1:
        ...
        st.info("This view compares a static (one-time train) model against an adaptive model that retrains monthly on new False Positives.")

        # --- 1. Consolidated Test Set Summary ---
        ...
        c1.metric("Test Period Duration", test_time_months_str)
        c2.metric("Total Events in Test Set", f"{len(test_df):,}" if test_df is not None else "0")
        c3.metric("Total Malicious Events", f"{total_tp:,}")
        ...
        metrics_holdout = pipeline_results["metrics_holdout"]
        metrics_fp_only = pipeline_results["metrics_fp_only"]
        
        with col1:
            st.subheader("Static (Holdout) Model")
            total_alerts_holdout = metrics_holdout['tp'] + metrics_holdout['fp']
            st.metric("Total Alerts", f"{total_alerts_holdout:,}")
            st.metric("Recall (Coverage)", f"{metrics_holdout['recall']:.2%}")
            st.metric("False Positives", f"{metrics_holdout['fp']:,}")

        with col2:
            st.subheader("Adaptive (FP-Retrain) Model")
            total_alerts_fp = metrics_fp_only['tp'] + metrics_fp_only['fp']
            # As requested, delta is REMOVED for Total Alerts and Recall
            st.metric("Total Alerts", f"{total_alerts_fp:,}")
            st.metric("Recall (Coverage)", f"{metrics_fp_only['recall']:.2%}")
            # As requested, delta is KEPT for False Positives
            st.metric("False Positives", f"{metrics_fp_only['fp']:,}",
                      delta=int(metrics_fp_only['fp'] - metrics_holdout['fp']), delta_color="inverse")
        
        st.markdown("---")

        # --- 3. Headline Metric: FP Reduction ---
        st.subheader("Impact of Adaptive Retraining")
        fp_holdout = metrics_holdout['fp']
        fp_adaptive = metrics_fp_only['fp']
        
        if fp_holdout > 0:
            fp_reduction_pct = ((fp_holdout - fp_adaptive) / fp_holdout)
            st.metric("False Positive Reduction", f"{fp_reduction_pct:.2%}",
                      help="The percentage decrease in false alarms due to monthly retraining. Higher is better.")
        else:
            st.info("The holdout model had no false positives, so reduction cannot be calculated.")
